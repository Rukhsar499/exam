...
try {
const res = await fetch("[https://njportal.thenoncoders.in/api/v1/get_joblist](https://njportal.thenoncoders.in/api/v1/get_joblist)", {
headers: {
...
// ✅ Submit form to API + Redirect to Easebuzz payment
...
return;
  ...
  if (result.status) {
    ...
    if (accessKey) {
      ...
      alert("Application submitted, but access key missing for payment.");
    }
  } else {
...
return ( <section className="py-16 bg-gray-50"> <div className="text-center mb-10 px-4"> <h2 className="text-3xl md:text-4xl font-bold text-[#000]">Current Job Openings</h2> </div>
        ...
        <div className="relative flex items-center justify-between mx-auto mb-6 px-4">
          {[1, 2, 3].map((step, index) => (
            <div key={step} className="flex items-center">
              ...
              {index < 2 && (
                <div className={`flex-1 h-[3px] md:w-[201px] w-[60px] ${currentStep > step ? "bg-blue-400" : "bg-gray-300"}`}></div>
              )}
            </div>
          ))}
        </div>
